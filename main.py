"""
BTK Yarışması 2025 - Ürün Analiz Sistemi v2.0

Bu uygulama, e-ticaret sitelerinden ürün verilerini çekerek AI destekli 
detaylı analiz yapan bir web uygulamasıdır.

🏆 BTK Yarışması 2025 için Trivox takımı tarafından geliştirilmiştir.
...
Özellikler:
- Çoklu platform desteği (Trendyol, Amazon vb.)
- Gerçek zamanlı web scraping
- AI destekli ürün analizi (Gemini AI)
- Yorum duygu analizi
- Ürün karşılaştırma
- Çoklu format export (JSON, CSV)
- Modern web arayüzü
...
# Logging (Günlük kaydı) ayarları - Hem dosyaya hem konsola yazdır
logging.basicConfig(
    level=logging.INFO,  # INFO seviyesi ve üstü logları kaydet
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',  # Log formatı
    handlers=[
        # Dosyaya yazma handler'ı - UTF-8 encoding ile Türkçe karakter desteği
...
# FastAPI uygulaması - Ana web API framework'ü
app = FastAPI(
    title="BTK Yarışması 2025 - Ürün Analiz Sistemi",
    description="AI destekli detaylı ürün analizi ve karşılaştırma sistemi | Trivox Takımı",
    version="2.0.0",
    contact={
        "name": "Trivox Takımı",
...
# Template engine - HTML şablonları için Jinja2
templates = Jinja2Templates(directory="templates")

# Statik dosyalar - CSS, JS, resim dosyaları için
app.mount("/static", StaticFiles(directory="static"), name="static")

# API anahtarı kontrolü - Gemini AI için gerekli
GEMINI_API_KEY = os.getenv('GEMINI_API_KEY')
if not GEMINI_API_KEY:
    logger.error("GEMINI_API_KEY environment variable bulunamadı!")
    GEMINI_API_KEY = "demo_key"  # Demo modunda çalışmak için
    
# Global nesneler - Uygulama boyunca kullanılacak ana sınıflar
scraper = ProductScraper()  # Web scraping için
detailed_analyzer = ProductDetailedAnalyzer(GEMINI_API_KEY)  # AI analiz için  
data_exporter = DataExporter()  # Veri export işlemleri için
...
async def api_status():
    """API durumu - BTK Yarışması Trivox Takımı"""
    try:
        saved_count = len(detailed_analyzer.get_all_product_ids())
        
        return JSONResponse({
            "status": "active",
            "project": "BTK Yarışması 2025 - Ürün Analiz Sistemi",
            "team": "Trivox",
            "team_members": [
                "Serhatcan Ünal - Proje Lideri & Backend",
                "Elif Zeynep Tosun - AI Uzmanı & Frontend", 
                "Meryem Gençali - Web Scraping & UI/UX"
            ],
            "version": "2.0.0",
            ...
            "competition": "BTK Hackathon 2025"
        })
        ...
        return JSONResponse({
            "status": "error",
            "error": str(e),
            "team": "Trivox - BTK Yarışması 2025"
        })
...
if __name__ == "__main__":
    ...
    logger.info("🏆 BTK Yarışması 2025 - Trivox Takımı")
    logger.info("🚀 Ürün Analiz Sistemi v2 başlatılıyor...")
    logger.info("👥 Takım: Serhatcan Ünal, Elif Zeynep Tosun, Meryem Gençali")
